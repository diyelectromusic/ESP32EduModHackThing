#define NUM_PWM_PINS 10
int pwm_pins[NUM_PWM_PINS] = {17,5,18,19,21,1,22,23,16,13};
...
//
// NB: As the PWM output is happening at half the sample rate
//     (it outputs on every other interrupt), we now need
//     need to double the frequency used in the increment too.
//
#define FREQ2INC(f) (f*4)
uint16_t acc[NUM_PWM_PINS];
uint16_t inc[NUM_PWM_PINS];
uint8_t *pWT[NUM_PWM_PINS] = {sinedata, sawdata, tridata, sqdata, sinedata, sawdata, tridata, sqdata, tridata, sawdata};
...
void ARDUINO_ISR_ATTR timerIsr (void) {
  ...
  if (toggle) {
    ...
    for (int i=NUM_PWM_PINS/2; i<NUM_PWM_PINS; i++) {
      ddsUpdate(i);
    }
  }
}
